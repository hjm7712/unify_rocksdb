...
// BIG SSD
void PartitionedFilterBlockBuilder::MaybeCutAFilterBlock_Unify(
    const Slice* next_key, size_t index_size) {

  // filter size (include current key)
  size_t filter_size = filter_bits_builder_->CalculateSize(keys_added_to_partition_ + 1);
...
//	  printf("f %ld nf %ld index %ld sum %ld\n", filter_size, next_filter_size, index_size, index_size+filter_size);
	  p_index_builder_->RequestUnifyCut();
  }
...
void PartitionedFilterBlockBuilder::Add_Unify(const Slice& key, size_t index_size) {
  MaybeCutAFilterBlock_Unify(&key, index_size);
  FullFilterBlockBuilder::Add(key);
}
