...
class IndexBuilder {
 public:
  ...
  // BIG SSD
  virtual void AddIndexEntry_Unify(std::string* last_key_in_current_block,
		  const Slice* first_key_in_next_block,
		  const BlockHandle& block_handle) = 0;
  virtual size_t CalculateSize() = 0;
  virtual void RequestUnifyCut() = 0;
  virtual bool CheckUnifyCut() = 0;
...
class ShortenedIndexBuilder : public IndexBuilder {
 public:
  ...
  virtual void AddIndexEntry_Unify(std::string* /*last_key_in_current_block*/,
		  const Slice* /*first_key_in_next_block*/,
		  const BlockHandle& /*block_handle*/) override {}
  virtual size_t CalculateSize() override {return 0;}
  virtual void RequestUnifyCut() override {}
  virtual bool CheckUnifyCut() override {return false;}
  ...
  void Finish_Unify(
		  IndexBlocks* index_blocks) { 
	  if (seperator_is_key_plus_seq_) {
		  index_blocks->index_block_contents = index_block_builder_.Finish_Unify();
	  } else {
		  index_blocks->index_block_contents =
			  index_block_builder_without_seq_.Finish_Unify();
	  }
	  return;
  }
...
class PartitionedIndexBuilder : public IndexBuilder {
 public:
  ...
  virtual void AddIndexEntry_Unify(std::string* last_key_in_current_block,
		  const Slice* first_key_in_next_block,
		  const BlockHandle& block_handle) override;
  virtual size_t CalculateSize() override;
  virtual void RequestUnifyCut() override;
  virtual bool CheckUnifyCut() override;
 ...
 private:
  ...
  // BIG SSD
  bool unify_cut_requested_ = false;
