...
// Big SSD
void BlockBasedTableBuilder::Add_Unify(const Slice& key, const Slice& value) {
  Rep* r = rep_;
  size_t cur_index_size = 0;
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
  ValueType value_type = ExtractValueType(key);
  if (IsValueType(value_type)) {
    ...
    if (r->state == Rep::State::kUnbuffered) {
      if (r->IsParallelCompressionEnabled()) {
        ...
        if (r->filter_builder != nullptr) {
          size_t ts_sz =
              r->internal_comparator.user_comparator()->timestamp_size();
          r->filter_builder->Add_Unify(ExtractUserKeyAndStripTimestamp(key, ts_sz), cur_index_size);
        }
      }
    }
...
void BlockBasedTableBuilder::WriteRawBlock_Unify(const Slice& index_contents,
                                           ...
                                           const Slice* raw_block_contents,
                                           bool is_top_level_filter_block) {
  // This function only for filter+index, not for data block
  /////////////////////////////////////////////////////////////////
  /////// filter block | index block | filter_size | trailer //////
  /////////////////////////////////////////////////////////////////
  Rep* r = rep_;
  StopWatch sw(r->ioptions.clock, r->ioptions.stats, WRITE_RAW_BLOCK_MICROS);
...
// BIG SSD
Status BlockBasedTableBuilder::Finish_Unify() {
  Rep* r = rep_;
  assert(r->state != Rep::State::kClosed);
  bool empty_data_block = r->data_block.empty();
  r->first_key_in_next_block = nullptr;
