...
struct CompactionJob::SubcompactionState {
  ...
  // Big SSD
  Status AddToBuilder_Unify(const Slice& key, const Slice& value) {
	  auto curr = current_output();
	  assert(builder != nullptr);
	  assert(curr != nullptr);
	  Status s = curr->validator.Add(key, value);
...
// Big SSD
void CompactionJob::ProcessKeyValueCompaction_Unify(SubcompactionState* sub_compact) {
  assert(sub_compact);
  assert(sub_compact->compaction);
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    status = sub_compact->AddToBuilder_Unify(key, value);
    if (!status.ok()) {
      break;
    }
    ...
    if (output_file_ended) {
      ...
      CompactionIterationStats range_del_out_stats;
      status = FinishCompactionOutputFile_Unify(input->status(), sub_compact,
                                          &range_del_agg, &range_del_out_stats,
                                          next_key);
      RecordDroppedKeys(range_del_out_stats,
                        &sub_compact->compaction_job_stats);
  ...
  if (sub_compact->builder != nullptr) {
    CompactionIterationStats range_del_out_stats;
    Status s = FinishCompactionOutputFile_Unify(status, sub_compact, &range_del_agg,
                                          &range_del_out_stats);
    if (!s.ok() && status.ok()) {
      status = s;
    }
...
// BIG SSD
Status CompactionJob::FinishCompactionOutputFile_Unify(
    const Status& input_status, SubcompactionState* sub_compact,
    CompactionRangeDelAggregator* range_del_agg,
    CompactionIterationStats* range_del_out_stats,
    const Slice* next_table_min_key /* = nullptr */) {
  ...
  const uint64_t current_entries = sub_compact->builder->NumEntries();
  if (s.ok()) {
    s = sub_compact->builder->Finish_Unify();
  } else {
    sub_compact->builder->Abandon();
  }
  IOStatus io_s = sub_compact->builder->io_status();
